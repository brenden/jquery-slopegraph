    ...
    return this.each(function() {
      ...
      function buildEntryList(entries, valueFirst) {
        ...
        entries.forEach(function(entry) {
          ...
          var entryOffset = position - entryHeight;

          // Check to see if this entry's optimal placement collides with an 
          // existing entry
          $newEntry.css('margin-top', entryOffset);
          var entryTop = $newEntry.offset().top;
          var entryBottom = entryTop + $newEntry.height();

          var $collidingEntry = $list.children('li').not($newEntry)
            .filter(function() {
              var otherTop = $(this).offset().top;
              var otherBottom = otherTop + $(this).height();

              return (entryTop >= otherTop && entryTop <= otherBottom) 
                || (entryBottom >= otherTop && entryBottom <= otherBottom);
            });
        });
